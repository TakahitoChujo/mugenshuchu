...
struct LogsView: View {
    ...
    @AppStorage("targetMinutesPerDay") private var targetMinutesPerDay: Int = 60 // ← SettingsView と同じキーを使用
    ...
    /// ✅ 連続日数（達成日ベース）
    /// - その日の「集中合計(分) >= 目標分」の日だけ +1
    /// - 同日中に何度達成しても +1 は当日のみ
    /// - 翌日は翌日に達成しないと伸びない
    private var streakDays: Int {
        // 日付→集中分の辞書（同日複数レコードがあっても合算）
        var focusMinByYMD: [String:Int] = [:]
        for l in logs {
            focusMinByYMD[l.ymd, default: 0] += l.focusSeconds / 60
        }

        var count = 0
        var cursor = Date() // 今日から遡る
        while true {
            let key = env.ymdString(from: cursor)
            let minutes = focusMinByYMD[key] ?? 0
            if minutes >= targetMinutesPerDay {
                count += 1
            } else {
                break
            }
    ...
    // サマリ
    private var totalFocusSec: Int { bins.reduce(0) { $0 + $1.focusSeconds } }
    private var totalBreakSec: Int { bins.reduce(0) { $0 + $1.breakSeconds } }
    private var totalSessions: Int { bins.reduce(0) { $0 + $1.sessions } }
    ...
    var body: some View {
        NavigationStack {
            ...
            .toolbar {
                NavigationLink {
                    SettingsView() // ← 既存の設定画面（AppStorageで目標分を編集）:contentReference[oaicite:4]{index=4}
                } label: {
            ...
            ForEach(bins) { bin in
                HStack {
                    ...
                    VStack(alignment: .trailing, spacing: 2) {
                        ...
                        Text("休憩 \(minutesText(bin.breakSeconds))").foregroundStyle(.secondary)
                        Text("回数 \(bin.sessions)").font(.caption).foregroundStyle(.secondary)
                    }
                    .font(.subheadline)
                    .monospacedDigit()
                }
    ...
    private func aggregate(logs: [DailyLog], by mode: AggregateMode, env: CalendarEnv) -> [PeriodBin] {
        ...
        for l in logs {
            ...
            case .week:
                ...
                var bin = dict[key] ?? PeriodBin(
                    key: key, label: "W\(week)",
                    startDate: start, endDate: end,
                    focusSeconds: 0, breakSeconds: 0, sessions: 0
                )

                bin = PeriodBin(
                    key: bin.key, label: bin.label,
                    startDate: bin.startDate, endDate: bin.endDate,
        ...
        return dict.values.sorted { a, b in a.startDate < b.startDate }
    }
